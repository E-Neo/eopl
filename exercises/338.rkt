...
(define-datatype expression expression?
  ...
  (nameless-proc-exp
   (body expression?))
  (cond-exp
   (left-exps (list-of expression?))
   (right-exps (list-of expression?))))
...
(define translation-of
  (lambda (exp senv)
    (cases expression exp
           ...
           (cond-exp (left-exps right-exps)
                     (cond-exp (map (lambda (exp1)
                                      (translation-of exp1 senv))
                                    left-exps)
                               (map (lambda (exp1)
                                      (translation-of exp1 senv))
  ...
  (lambda (exp nameless-env)
    (cases expression exp
           ...
           (cond-exp (left-exps right-exps)
                     (letrec
                         ((value-of-cond
                           (lambda (left-exps right-exps)
                             (if (null? left-exps)
                                 (eopl:error 'cond
                                             "None of the tests succeeds:~%~s"
                                             exp)
                                 (if (expval->bool (value-of (car left-exps)
                                                             nameless-env))
                                     (value-of (car right-exps) nameless-env)
                                     (value-of-cond (cdr left-exps)
                                                    (cdr right-exps)))))))
                       (value-of-cond left-exps right-exps)))
